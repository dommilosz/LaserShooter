...
export const selectedClientContext = createContext<[number, React.Dispatch<React.SetStateAction<number>>]>();
...
export default function ClientsView(){
    ...
    return <div style={{display: "flex", height: "calc( 100% - 80px )"}}>
        <div style={{display: "flex", width: "33%"}}>
            <ClientsList selectedShot={selectedClient} setSelectedShot={setSelectedClient}/>
        </div>
        <div style={{display: "flex", width: "66%", height: "calc( 100% - 4px )", flexDirection: "column"}}>
            ...
            <TargetVisualuser
                shots={sessionData.shots.filter((shot)=>{
                    return shot.idPacket.clientId === Number(Object.keys(sessionData.clients)[selectedClient]);
                })}
                dotColor={"red"}
                secondaryColor={"red"}
            ></TargetVisualuser>
