...
export default function ClientsList({selectedShot: selectedClient,setSelectedShot: setSelectedClient}:any) {
    let {sessionInfo, sessionData, users} = useContext(sessionContext);

    return <selectedClientContext.Provider value={[selectedClient, setSelectedClient]}>
        <div className="object-container">
            {Object.keys(sessionData.clients).map((clientKey, i) => {
                let client = sessionData.clients[clientKey as unknown as number];
                return <ClientObject client={client} index={i}></ClientObject>
            })}
        </div>
    </selectedClientContext.Provider>
}

export function ClientObject({client, index}: { client: ClientData, index: number }) {
    ...
    return <div
        className={"shot_object" + (selectedClient == index ? " active" : "")}
        onClick={() => {
        ...
        <UserAssignModal
            elements={Object.keys(users).map((user => {
                return {name: users[user as unknown as number], value: user}
            }))} title={"Select User to associate with this client id"}
            callback={(value) => {
                fetch(`http://localhost:8008/client/${client.id}/user`,{method: 'PUT',body:JSON.stringify({"from":+new Date(),"newUser":value}),headers:{"content-type":"application/json"}});
            }}
